...
(function () {
    ...
    function twoColumnMenuGenerate() {
        ...
        if (defaultValues && (isTwoColumn == "twocolumn" || defaultValues["data-layout"] == "twocolumn")) {
            ...
            ul.innerHTML = '<a href="#" class="logo"><img src="images/logo-dark.png" alt="" height="22"></a>';
            ...
            var currentPath = getLastPathSegment(getCurrentPathname());
            if (currentPath) {
                // navbar-nav
                var a = findSidebarMenuLink(currentPath);
            ...
            Array.from(document.querySelector("#two-column-menu ul").querySelectorAll("li a")).forEach(function (element) {
                var currentPath = getLastPathSegment(getCurrentPathname());
                element.addEventListener("click", function (e) {
                    if (!(isCurrentPath(element.getAttribute("href")) && !element.getAttribute("data-bs-toggle")))
    ...
    function isCurrentPath(href) {
        var hrefPath = getPathnameFromHref(href);
        return !!hrefPath && hrefPath === getCurrentPathname();
    }

    function findSidebarMenuLink(currentPath) {
        var navbar = document.getElementById("navbar-nav");
        if (!navbar) return null;
        var normalizedCurrentPath = getCurrentPathname();

        return Array.from(navbar.querySelectorAll("a[href]")).find(function (link) {
            var normalizedHrefPath = getPathnameFromHref(link.getAttribute("href"));
            if (!normalizedHrefPath) return false;
            if (normalizedHrefPath === normalizedCurrentPath) return true;
            return getLastPathSegment(normalizedHrefPath) === currentPath;
        }) || null;
    }

    function findCurrentTwoColumnMenuLink() {
        var twoColumnMenu = document.getElementById("two-column-menu");
        if (!twoColumnMenu) return null;

        return Array.from(twoColumnMenu.querySelectorAll("a[href]")).find(function (link) {
            return isCurrentPath(link.getAttribute("href"));
        }) || null;
    }
    ...
    function initTwoColumnActiveMenu() {
        ...
        if (currentPath) {
            if (document.body.className == "twocolumn-panel") {
                var currentTwoColumnLink = findCurrentTwoColumnMenuLink();
                if (currentTwoColumnLink) {
                    currentTwoColumnLink.classList.add("active");
                }
            }
            // navbar-nav
            var a = findSidebarMenuLink(currentPath);
    ...
    function setDefaultAttribute() {
        if (!localStorage.getItem("defaultAttribute")) {
            ...
            // keep theme customizer closed by default
            // users can open it anytime using the floating button
        } else {
            var isLayoutAttributes = {};
